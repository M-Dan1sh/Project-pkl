    ...
    public function loadExamDashboard($id)
    {
        ...
        if(count($qnaExam) > 0){
            ...
            if ($attemptCount >= $qnaExam[0]['attempt']) {
                return view('student.exam-dashboard',['success'=>false,'msg'=>'Percobaan ujian telah habis','exam'=>$qnaExam]);
            }
                ...
                if (count($qnaExam[0]['getQnaExam']) > 0) {
                    ...
                    return view('student.exam-dashboard',['success'=>false,'msg'=>'Ujian ini akan mulai sebentar lagi','exam'=>$qnaExam]);
                }
                

            } else if($qnaExam[0]['date'] > date('Y-m-d')){
                return view('student.exam-dashboard',['success'=>false,'msg'=>'Ujian ini akan di mulai pada '.$qnaExam[0]['date'],'exam'=>$qnaExam]);
            } else{
                return view('student.exam-dashboard',['success'=>false,'msg'=>'Masa berlaku ujian sudah berakhir pada '.$qnaExam[0]['date'],'exam'=>$qnaExam]);
            }
        }
        else{
            return view('404');
